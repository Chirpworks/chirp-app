...
def transcribe_and_diarize(audio_path, compute_type="float32"):
    ...
    try:
        ...
        diarize_model = whisperx.DiarizationPipeline(use_auth_token=Config.WHISPERX_TOKEN, device=device)
...
def format_results(result):
    ...
    for segment in result["segments"]:
        ...
        formatted_segment = {
            'start': round(segment.get('start', -1), 2),
            'end': round(segment.get('end', -1), 2),
            'speaker': segment.get('speaker', 'UNKNOWN'),
            'text': segment.get('text', ''),
            'words': [{
                'word': word.get('word', ''),
                'start': round(word.get('start', -1), 2),
                'end': round(word.get('end', -1), 2),
